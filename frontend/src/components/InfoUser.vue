      ...
      <div class="d-flex flex-column flex-lg-row align-items-center">
        <div class="d-flex position-relative me-lg-3">
          <div class="photo border border-primary rounded">
            <img alt="photo de profil" v-if="userProfil.Ppicture"  
            :src="userProfil.Ppicture"/>
            <img alt="photo de profil par défaut" v-else class="" 
        ...
        <ul class="list-group-flush p-0 mb-0 w-100">
          ...
          <li :class="list">
            <div class="d-flex justify-content-start align-items-center w-100">
              <div class="flex-shrink-0 me-2">Nom :</div> <div v-if="modifyToggle == false" 
              class="ms-1">{{ userProfil.surname }}</div>
              <input v-if="modifyToggle == true" type="text" 
              class="form-control" name="nom" v-model="userProfil.surname">
            </div>
          </li>
          <li :class="list">
            <div class="d-flex justify-content-start align-items-center w-100">
              <div class="flex-shrink-0 me-2">Prénom :</div>
              <div v-if="modifyToggle == false" class="ms-1">
                {{ userProfil.name }}
              </div>
              <input v-if="modifyToggle == true" type="text" 
              class="form-control" name="prénom" v-model="userProfil.name">
            </div>
          </li>
          <li :class="list">
            <div class="d-flex justify-content-start align-items-center w-100">
              <div class="flex-shrink-0 me-2">Service :</div><div v-if="modifyToggle == false" class="ms-1">
                {{ userProfil.service }}
              </div>
              <select v-if="modifyToggle == true" 
              class="form-select" v-model="userProfil.service" aria-label="Default select example">
          ...
          <li :class="list">
            <div  class="d-flex justify-content-start align-items-center w-100">
              <div class="flex-shrink-0 me-2">Adresse Mail :</div>
              <div v-if="modifyToggle == false" class="ms-1">
                {{ userProfil.email }}
              </div>
              <input v-if="modifyToggle == true"  type="text" 
              class="form-control" name="email"  v-model="userProfil.email">
            </div>
          </li>
          <li :class="list">
...
<!-- MODIFY PROFIL -->
      <div v-if="userProfil.id == user || $store.state.user.admin == true" 
      class="d-flex flex-column flex-lg-row justify-content-end align-items-center py-3">
        <button v-if="modifyToggle == false" @click="modifyProfil()" 
          class="btn btn-secondary border border-primary w-100 my-2 me-lg-2">
          Modifier le profil
        </button>
        <button v-if="modifyToggle == true" @click="saveNewProfil()" 
          class="btn btn-secondary border border-primary w-100 text-nowrap my-2 me-lg-2">
          Enregistrer les modifications
        </button>
        <input @change="addPhoto()" type="file" ref="Ppicture" id="modifyPhoto" class="inputfile"/>
        <button @click="modifyPhoto()" v-if="userProfil.id == user || $store.state.user.admin == true" 
          class="btn btn-secondary border border-primary w-100 my-2 me-lg-2" for="modifyPhoto">
          Modifier la photo
        </button>
        <button class="btn btn-secondary border border-primary w-100 my-2 me-lg-2" 
          role="button" @click="modifyPassword()">
          Changer le mot de passe
        </button>
        <button class="btn btn-secondary border border-primary w-100 my-2" 
          role="button" @click="deleteAccount()">
          Supprimer le compte
        </button>
      </div >
<!-- MODIFY PASSWORD -->
      <form v-if="passwordToggle == true" class="mt-4 w-50 align-self-center">
        <div class="my-3">
          ...
          class="form-control" id="Password" @focus="password.nonEqual = false; password.invalid = false">
        </div>
        <div class="mb-3">
          <label for="PasswordVerif" class="form-label">Confirmer votre nouveau mot de passe</label>
          <input v-model="newPasswordVerif" type="password" 
          class="form-control" id="PasswordVerif" @focus="password.nonEqual = false; password.invalid = false">
        </div>
        <p v-if="password.invalid" class="text-danger">
          Le mot de passe est invalide, il doit contenir au moins 8 caractères, des chiffres, des majuscules et minuscules.
        </p>
        <p v-if="password.nonEqual" class="text-danger">
          Les mots de passe ne correspondent pas. Vérifier votre saisie.
        </p>
        <button @click="saveNewPassword" type="button" 
        class="btn btn-secondary border border-primary">
          Changer le mot de passe
        </button>
      </form>
<!-- DELETE ACCOUNT -->
      <div v-if="deleteToggle" class="mt-5 d-flex flex-column">
        ...
        class="btn btn-secondary border border-primary mb-3">Annuler</button>
        <button type="button" @click="deletingAccount()" 
        class="btn btn-light border border-primary mb-3">J'ai bien compris, je souhaite supprimer mon compte</button>
      </div>   
    </div>
<!-- MESSAGE POPUP -->
    <transition name="fade">
      <div v-if="userMessage" id="userMessage" 
      class="userMessage d-flex position-fixed justify-content-center align-items-center w-100 h-100">
  ...
  methods:{
    ...
    saveNewProfil(){ // SAUVEGARDE DU PROFIL MODIFIÉ
      ...
      if(this.userProfilId != this.user && this.$store.state.user.admin == true){
        ...
        surname: this.userProfil.surname, 
        name: this.userProfil.name, 
        service: this.userProfil.service, 
        Ppicture: this.userProfil.photo, 
        password: this.userProfil.password}, { headers: { Authorization: 'bearer ' + localStorage.getItem('token') } })
